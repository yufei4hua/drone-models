...
def quat2euler(quat: NDArray[np.floating], seq: str, degrees: bool = False) -> NDArray[np.floating]:
    ...
    angles = _at(
        angles, 0, xp.where(case == 1, 2 * half_sum, 2 * half_diff * xp.where(extrinsic, -1, 1))
    )  # any degenerate case
    # angles = angles.at[angle_first].set(xp.where(case == 0, half_sum - half_diff, angles[angle_first]))
    angles = _at(
        angles, angle_first, xp.where(case == 0, half_sum - half_diff, angles[..., angle_first])
    )
    # angles = angles.at[angle_third].set(xp.where(case == 0, half_sum + half_diff, angles[angle_third]))
    angles = _at(
        angles, angle_third, xp.where(case == 0, half_sum + half_diff, angles[..., angle_third])
    )
    # angles = angles.at[angle_third].set(xp.where(symmetric, angles[angle_third], angles[angle_third] * sign))
    angles = _at(
        angles,
        angle_third,
        xp.where(symmetric, angles[..., angle_third], angles[..., angle_third] * sign),
    )
...
def _at(array: NDArray, index: int, new_value: float):
    ...
    return xp.concat(
        [array[..., :index], xp.expand_dims(new_value, axis=-1), array[..., index + 1 :]], axis=-1
    )
