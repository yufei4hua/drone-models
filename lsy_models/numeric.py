...
def f_first_principles(
    state: QuadrotorState, command: NDArray[np.floating], constants: Constants
) -> QuadrotorState:
    ...
    """
    xp = state.pos.__array_namespace__()  # This looks into the type of the position array and decides what implementation to use (numpy, jax, etc)
    rot = R.from_quat(state.quat)

    # Thrust dynamics
    forces_motor_dot = constants.KD * (command - state.forces_motor)
    # Creating force and torque vector
    forces_motor_tot = xp.sum(state.forces_motor, axis=-1)
    ...
    torques_motor_vec = (state.forces_motor @ constants.SIGN_MATRIX) * xp.array(
    ...
    if state.forces_dist is not None:
        force_world_frame = (
            forces_motor_vec_world + constants.GRAVITY_VEC * constants.MASS + state.forces_dist
        )
    else:
    ...
    x, y, z = xp.split(state.angvel, 3, axis=-1)
    ...
    xi1 = xp.insert(-state.angvel, 0, 0, axis=-1)  # First line of xi
    xi2 = xp.concat((xp.expand_dims(state.angvel.T, axis=0).T, -angvel_skew), axis=-1)
    xi = xp.concat((xp.expand_dims(xi1, axis=-2), xi2), axis=-2)
    if state.torques_dist is not None:
        # paper: rot.as_matrix() @ torques_dist
        torques = torques_motor_vec + rot.apply(state.torques_dist)
    else:
        torques = torques_motor_vec
    quat_dot = 0.5 * (xi @ state.quat[..., None]).squeeze(axis=-1)
    angvel_dot = (
        torques - xp.cross(state.angvel, state.angvel @ constants.J)
    ) @ constants.J_inv  # batchable version

    return QuadrotorState.create(pos_dot, vel_dot, quat_dot, angvel_dot, forces_motor_dot)
